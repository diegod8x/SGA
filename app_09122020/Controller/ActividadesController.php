 ...
 */
	public function view($id = null) {		
		...
		if(trim($actividad["Actividade"]["direccion"])!='')
	...
	public function add_actividad(){
		...
		if(array_key_exists("id", $this->request->data["Actividade"])){ 
			...
			if($respuesta["estado"]==1){
				...
				if(!empty($productosNuevos)){
					...
					if(!empty($descProductos)){
						foreach ($descProductos as $descripcion) {
							$descProducto[$descripcion["Producto"]["id"]] = array("nombre"=>$descripcion["Producto"]["nombre"],
				...
				$Email->from(array('info@camas.cl' => 'SGA'));
				$Email->to($trabajador["Trabajadore"]["email"]); //datosCorreo["email"]
				//$Email->to('polaco2k@gmail.com');
				$Email->cc('actividades@camas.cl');
				//$Email->cc('diegodiazr@vtr.net');
 ...
 */
	...
	public function data_json(){
		...
		foreach ($clientes as $cliente) {
			...
			foreach ($cliente["Contrato"] as $key => $value) {				
				$listadoContratos[$cliente["Cliente"]["id"]] =  array_merge($cliente["Contrato"][$key], 
	...
	public function listado_consolidado($fInicio = null, $fTermino = null){
		...
		$tipoActividades = $this->TipoActividade->find("list",  array("fields"=>array("nombre"),"recursive"=>0));			
		...
		if($fInicio){
			$conditions = array("Actividade.fecha_ingreso >=" => $fInicio,
		...
		if(!empty($actividades)){
			foreach ($actividades as $key => $actividad) {
				...
				$listadoActividades[] = array_merge($actividad["Actividade"], array(
					...
					"nombre_cliente"=> (isset($actividad["Actividade"]["cliente_id"]))? $nombreClientes[$actividad["Actividade"]["cliente_id"]]: ''
					));
			}						
		}else{
