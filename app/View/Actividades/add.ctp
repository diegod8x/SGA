<div ng-controller="actividadesAdd" ng-init="obtDatos(<?php if (isset($this->request->pass)) {
																												echo implode(",", $this->request->pass);
																											} ?>)">
		...
		<div class="container-fluid">
			<div class="row">
				...
				<form role="form" class="form-horizontal" name="actividadesAdd" novalidate>
					...
					<div class="form-group sube" ng-class="{ 'has-error': !actividadesAdd.rangoEntrega.$valid }">
						<label class="control-label col-md-3 baja">Rango de Entrega</label>
						<div class="col-md-6">
							<input class="form-control" type="text" name="rangoEntrega" placeholder="Rango de Entrega" ng-model="formulario.Actividade.rango_entrega">
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-md-6">
							<ui-select ng-model="formulario.Actividade.cliente_id" on-select="obtDirecciones($select.selected.id)" name="Clientes" theme="bootstrap" required ng-disabled="{{disableCliente}}" disabled="{{disableCliente}}">
