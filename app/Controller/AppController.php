...
class AppController extends Controller
{
	...
	public function beforeFilter() {
        ...
        if ($this->request->is('post'))
        {
			...
			{				
				$nombreUser = $this->User->find("first", array( "conditions"=>array("User.usuario"=>$this->data["users"]["usuario"],
				...
				if(!empty($nombreUser)){
			    	$this->Session->write('Users', array( "nombre" =>$nombreUser["User"]["nombre"], 'usuario'=>$this->data["users"]["usuario"], 'flag' => 1));
				}
			    else{
