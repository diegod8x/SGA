 ...
 */
	...
	public function listado_productos(){
		...
		$this->loadModel("Contrato");
		$this->loadModel("ContratosProducto");
		$nameDb = ConnectionManager::$config->default['database'];
		...
		FROM $nameDb.`contratos_productos` AS `ContratosProducto` 
		LEFT JOIN $nameDb.`contratos` AS `Contrato` ON (`ContratosProducto`.`contrato_id` = `Contrato`.`id`) 
		LEFT JOIN $nameDb.`productos` AS `Producto` ON (`ContratosProducto`.`producto_id` = `Producto`.`id`) 
