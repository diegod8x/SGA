...
class DireccionesController extends AppController
{
	...
	public function listado_direcciones()
	{
		...
		$direcciones = $this->Direccione->find("all", array(
			"fields" => array(
				"Direccione.nombre", "Direccione.estado", "Comuna.nombre", "Regione.nombre", "Cliente.nombre", "Cliente.rut"
			),
	...
	public function migrar()
	{
		...
		$contratosId = $this->Contrato->find(
			"list",
			array(
				"fields" => array("id"),
				"conditions" => array(
		...
		$clientes = $this->Cliente->find("all", array(
			...
			"conditions" => [
				array("Cliente.id IN" => $contratosId),
			],
			"recursive" => 0
		));
			...
			if (isset($last["Direccione"]["id"])) {
				...
				if (!empty($contratos)) {
					foreach ($contratos as $contrato) {
						$contrato["Contrato"]["created_user"] = "migrado";
		...
		pr(count($contratosActualizados));
