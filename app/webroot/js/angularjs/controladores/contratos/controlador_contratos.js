app.controller('contratosIndex',  ['$scope', '$rootScope','$http', '$filter', '$location', 'uiGridConstants','i18nService', function ($scope, $rootScope, $http, $filter, $location, uiGridConstants, i18nService) {
    $scope.loader = true;
    ...
    $scope.gridOptions.columnDefs = [
        ...
        {name:'cantidad_productos', displayName:'Cant. Productos', cellTemplate: '<div style="text-align:center;"  class="ngCellText">{{row.entity.cantidad_productos}}</div>', minWidth:110},
        ...
        {name:'costo_despacho', displayName:'Despacho', cellTemplate: '<div style="text-align:right;"  class="ngCellText">{{row.entity.costo_despacho | number:0}}</div>', minWidth:110},                
        {name:'precio_total', displayName:'Total', cellTemplate: '<div style="text-align:right;"  class="ngCellText">{{row.entity.precio_total | number:0}}</div>', minWidth:110},
    ];
...
app.controller('contratosAdd', function ($scope, $http, contratosService, serviciosService, productosService,  Flash, $filter, $window, $location, $rootScope, $anchorScroll) {
    $scope.loader = true;
    $scope.cargador = loader;
    ...
    $scope.obtDatos = function(){    
        ...
        contratosService.getData().success(function (data){
            ...
            angular.element("#fechaInicio").datepicker("setDate", data.fecha_inicio);
        ...
        $scope.$watch('formulario.Contrato.tipo_contrato_id', function(datos){
        ...
        $scope.$watch('formulario.Contrato.fecha_inicio', function(anterior,nuevo){            
        ...
        $scope.$watch("formulario.Contrato.fecha_cobro", function(anterior){
            if(angular.isDefined(anterior))
                if(anterior>28) 
                    $scope.formulario.Contrato.fecha_cobro = '';
        });

        $scope.$watchGroup(['formulario.Contrato.descuento','formulario.Contrato.costo_despacho','formulario.Contrato.subtotal', 'formulario.Contrato.numero_cuota_id'], function(cant){
            if(angular.isDefined($scope.formulario.Contrato.costo_despacho))
                $scope.formulario.Contrato.costo_despacho = parseInt($scope.formulario.Contrato.costo_despacho);
            if(angular.isDefined($scope.formulario.Contrato.descuento))
                $scope.formulario.Contrato.descuento = parseInt($scope.formulario.Contrato.descuento);
            ...
            $scope.formulario.Contrato.precio_total = $scope.formulario.Contrato.subtotal - $scope.formulario.Contrato.descuento + $scope.formulario.Contrato.costo_despacho;      
            $scope.formulario.Contrato.valor_cuota = Math.round( (Number($scope.formulario.Contrato.precio_total) / Number($scope.numeroCuota)) * 100)/100;
        });
    };
    $scope.setProducto = function (idProducto){
        productosService.getProducto(idProducto).success(function (data){        
            ...
            $scope.$watchGroup(['Producto.precio_venta','Producto.precio_arriendo','Producto.cantidad'], function(data){      
                if(angular.isDefined($scope.Producto)){
                    ...
                    if($scope.arriendo){
                        ...
                        if(angular.isDefined($scope.Producto.cantidad) && angular.isDefined($scope.Producto.precio_arriendo))
                            $scope.Producto.subtotal = Number($scope.Producto.precio_arriendo) * Number($scope.Producto.cantidad);
                    }else{
                        $scope.Producto.precio_venta = valores[0];
                        $scope.Producto.precio_arriendo = 0;
                        if(angular.isDefined($scope.Producto.cantidad) && angular.isDefined($scope.Producto.precio_venta))
                            $scope.Producto.subtotal = Number($scope.Producto.precio_venta) * Number($scope.Producto.cantidad);
                    }  
                }
            });
    ...
    $scope.recalcular = function(form){
        ...
        $scope.formulario.Contrato.subtotal = 0;
        console.log('entra');
        angular.forEach($scope.formulario.ContratosProducto, function(value, id){
            console.log('entra2');
            console.log(value);
            $scope.formulario.Contrato.cantidad_productos = Number($scope.formulario.Contrato.cantidad_productos) + Number(value.cantidad);
            $scope.formulario.Contrato.subtotal = Number($scope.formulario.Contrato.subtotal) + Number(value.subtotal);
        }); 
    }
    ...
    $scope.registrarContrato = function(){     
        ...
        var productosNuevos = [];
        angular.forEach($scope.formulario.ContratosProducto, function(value, id){
            if(value.pendiente == 1)
                productosNuevos.push(value.producto_id);
        });
    	contratosService.registrarContrato($scope.formulario).success(function (data){            
            if(data.estado==1){
...
app.controller('contratosEdit', function ($scope, $http, contratosService, productosService, serviciosService, Flash, $filter, $window, $location, $rootScope, $anchorScroll) {
    $scope.loader = true;
    $scope.cargador = loader;
    $scope.formContratos = false;
    var Contrato;
    ...
    $scope.obtDatos = function(idContrato){ 
        ...
        console.log(data);
            ...
            $scope.formulario.Contrato.fecha_inicio = data.data.Contrato.fecha_inicio.split("-").reverse().join("-");
            if($scope.formulario.Contrato.tipo_contrato_id==1) {
                $scope.arriendo = true;
                $scope.venta = false;
                $scope.formulario.Contrato.precio_venta = 0;
            ...
            var valores = [];
            $scope.$watchGroup(['formulario.Contrato.subtotal','formulario.Contrato.costo_despacho','formulario.Contrato.descuento', 'formulario.Contrato.numero_cuota_id'], function(cant){
                angular.forEach(cant, function(ingreso,key){       
                    valores[key] = parseInt(Number(ingreso));                        
                }); 
                ...
                if( angular.isDefined($scope.formulario.Contrato.subtotal) && angular.isDefined($scope.formulario.Contrato.costo_despacho) && angular.isDefined($scope.formulario.Contrato.descuento) )
                    $scope.formulario.Contrato.precio_total = (Number($scope.formulario.Contrato.subtotal) + Number($scope.formulario.Contrato.costo_despacho)) - Number($scope.formulario.Contrato.descuento);
                $scope.formulario.Contrato.valor_cuota = Math.round( (Number($scope.formulario.Contrato.precio_total) / Number($scope.numeroCuota)) * 100)/100;
            });  
        });           
    };
    ...
    $scope.eliminarProductoCarro = function(index){
        ...
        if(angular.isDefined($scope.formulario.ContratosProducto[index].pendiente))
            $scope.formulario.ContratosProducto.splice(index,1);
        else
            $scope.formulario.ContratosProducto[index].estado = 0;
    ...
    $scope.contratoCliente = function(){
        ...
        contratosService.correoCliente($scope.formulario).success(function (data){            
            if(data.estado==1){
                Flash.create('success', data.mensaje, 'customAlert');
